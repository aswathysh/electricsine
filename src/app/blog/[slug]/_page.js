...
import { useBlogDetails, useBlogs } from "@/services/PracticeQueris";
import "./slug.css";
...
export default function BlogDetailPage() {
  ...
  const { data: blogListDatas, isLoading } = useBlogDetails(slug);
  ...
  useEffect(() => {
    ...
    const all = blogListDatas?.data || null;
    // const found = all.find((a) => a.slug === slug);
    const rest = (allBlogs?.data || []).filter((a) => a.slug !== slug);
     setArticle(all || null);
    setRelated(rest);
  }, [blogListDatas, allBlogs,slug]);
