...
export const fetchPracticeQuestions = async ({ pageParam,subjectId }) => {
  ...
  //   const response = await axiosInstance.get(`https://rubiksoftwares.com/electricsine-api/public/api/exam-list/${subjectId}`);
  //   return response.data;
  // };
...
export async function fetchExamSectionsParam() {
  ...
  const response = await fetch(`https://rubiksoftwares.com/electricsine-api/public/api/exam-list/${subjectId}`);
  const data = await response.json();
  return data;
}
export async function fetchExamSectionsParams() {
  try {
    const response = await fetch('https://rubiksoftwares.com/electricsine-api/public/api/exam-list/1');
